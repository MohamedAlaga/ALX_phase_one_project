...
export default function buyTab() {
  ...
  var [buyPrice, setBuyPrice] = useState<string>("0");
  var [itemsList, setItemsList] = useState<Array<any>>([]);

  const getRecieptsNumber = async () => {
    try {
      let response = await fetch("http://localhost:8000/api/purchase/last", {
        method: "GET",
        headers: {
  ...
  const getItemByBarcode = async (barcode: string) => {
    try {
      ...
      item["quantity"] = count;
      item["price"] = parseFloat(buyPrice);
      return item;
    } catch (error) {
      return {};
    }
  ...
  async function getReciepts(number: number) {
    try {
      ...
      let response = await fetch("http://localhost:8000/api/purchase/" + number, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <main className="py-6 px-12">
      <div className="grid grid-rows-2 grid-cols-6 text-3xl items-center place-items-center gap-6 py-6">
        ...
        <input className="w-full h-16 border-solid border-violet-600 border-2 rounded-md col-span-2 text-center"
          type="text"
          value={isLoading ? "Loading..." : recieptsNumberRef.current || ""}
          disabled />
        <p>barcode</p>
        <input className="w-full h-16 border-solid border-violet-600 border-2 rounded-md col-span-2  px-2"
          type="text"
          value={barcode}
        ...
        <p>buy price</p>
        <input
          className="w-full h-16 border-solid border-violet-600 border-2 rounded-md px-2 text-center"
          type="text"
        ...
        <button className="h-16 bg-violet-600 rounded-md w-full py-3 flex  items-center justify-center text-white text-2xl col-span-2" onClick={handleAdd}>Add</button>
      </div>
      ...
      <div className="grid grid-rows-2 grid-cols-6 text-3xl items-center place-items-center gap-6 py-6">
        <button className="h-16 bg-violet-600 rounded-md w-full py-3 flex text-2xl items-center justify-center text-white " onClick={handlePrevious}>Previous</button>
        <button className="h-16 bg-violet-600 rounded-md w-full py-3 flex text-2xl items-center justify-center text-white " onClick={handleNext}>Next</button>
        <div></div>
        <div></div>
        <button className="h-16 bg-violet-600 rounded-md w-full py-3 flex text-2xl items-center justify-center text-white col-span-2" onClick={purchaseItems}>Save</button>
      </div>
